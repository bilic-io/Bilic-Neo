/**
 * Export and Reporting Utilities for Delegate
...
interface AutoTableOptions {
  ...
  startY?: number;
  theme?: string;
  [key: string]: any; // For any other properties that might be used
}
...
export const exportConversation = async (
  ...
  const filename = title || `delegate-report-${new Date().toISOString().split('T')[0]}`;
...
const exportToPdf = async (
  ...
  // Add title
  doc.setFontSize(20);
  doc.setTextColor(34, 197, 94); // #22c55e - Delegate green
  doc.text('Delegate Compliance Report', 14, 20);
  ...
  doc.autoTable({
    ...
    headStyles: {
      fillColor: [34, 197, 94], // #22c55e - Delegate green
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(
      'Delegate - Compliance Reporting System',
...
const exportToText = (messages: MessageType[], options: ExportOptions, filename: string): void => {
  ...
  // Add title
  output += '======== DELEGATE COMPLIANCE REPORT ========\n\n';
...
const exportToJSON = (messages: MessageType[], options: ExportOptions, filename: string): void => {
  const exportData = {
    metadata: {
      title: options.title || 'Delegate Compliance Report',
...
export const generateExecutiveSummary = (messages: MessageType[], companyInfo?: CompanyInfo): string => {
  ...
  summary += `## Overview\n\n`;
  summary += `This report summarizes the compliance conversation with Delegate Assistant. `;
  ...
  summary += `\n## Conclusion\n\n`;
  summary += `This summary was automatically generated by Delegate and may not capture all compliance details. `;
